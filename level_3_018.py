...
# Question: A website requires the users to input username and password to
...
# each separated by a comma. Example If the following passwords are given as
# input to the program: ABd1234@1,a F1#,2w3E*,2We3345 Then, the output of
...
for password in passwords:
    if len(password) < 6 or len(password) > 12:
        ...
        test = 0
        if [True for x in password if x in string.ascii_letters.lower()]:
            test += 1
        if [True for x in password if x in string.ascii_letters.upper()]:
            test += 1
        if [True for x in password if x in string.digits]:
            test += 1
        if [True for x in password if x in '$#@']:
            test += 1

    if test == 4:
        corect_pass += password+", "
